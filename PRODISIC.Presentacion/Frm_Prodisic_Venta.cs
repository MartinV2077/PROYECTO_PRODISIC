...
{
    public partial class Frm_Prodisic_Venta : Form
    {
        public Frm_Prodisic_Venta()
        {
            InitializeComponent();
        }
        ...
        private void Formato_pv()
        {
            ...
            Dgv_Listado.Columns[1].HeaderText = "PRODISIC VENTA";
        }
        ...
        private void Listado_pv(string cTexto)
        {
            try
            {
                Dgv_Listado.DataSource = N_Prodisic_Venta.Listado_pv(cTexto);
                this.Formato_pv();
        ...
        private void Frm_Prodisic_Venta_Load(object sender, EventArgs e)
        {
            this.Listado_pv("%");
        }
        ...
        private void Btn_guardar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string Rpta = "";
                    E_Prodisic_Venta oPropiedad = new E_Prodisic_Venta();
                    oPropiedad.Codigo_pv = this.nCodigo;
                    oPropiedad.Descripcion_pv = Txt_descripcion.Text.Trim();
                    Rpta = N_Prodisic_Venta.Guardar_pv(this.Estadoguarda,oPropiedad); 
                    if (Rpta.Equals("OK"))
                    {
        ...
        private void Btn_eliminar_Click(object sender, EventArgs e)
        {
            if (Dgv_Listado.Rows.Count > 0)
            {
                ...
                if (Opcion==DialogResult.Yes)
                {
                    string Rpta = "";
                    this.Selecciona_item();
                    Rpta = N_Prodisic_Venta.Eliminar_pv(this.nCodigo);
                    if (Rpta.Equals("OK"))
                    {
                        this.Listado_pv("%");
        ...
        private void Btn_reporte_Click(object sender, EventArgs e)
        {
            if (Dgv_Listado.Rows.Count > 0)
            {
                Reportes.Frm_Rpt_Prodisic_Venta oRpt_pv = new Reportes.Frm_Rpt_Prodisic_Venta();
